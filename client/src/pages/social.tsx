...
import { CameraIcon, ImageIcon, RefreshCw, Send, Upload } from "lucide-react";
...
function SocialCard({ post }: { post: SocialMedia | any }) {
  ...
  // Debug status
  console.log(
    `Post ${id} status: ${status}, cleaned: ${cleanStatus}, isPending: ${isPending}, isPosted: ${isPosted}`,
  );
      ...
      <CardContent className="px-6 py-4">
        ...
        {isAIGenerated && imageUrl && (
          <div className="mb-4">
            ...
            {caption && <p className="text-sm text-neutral-800">{caption}</p>}
          </div>
        )}
          ...
          <div className="mt-4 bg-neutral-50 p-3 rounded-md text-sm">
            <p className="font-medium mb-1 flex items-center">
              ...
              <span className="text-xs text-neutral-500 ml-2">
                {aiRespondedAt &&
                  format(new Date(aiRespondedAt), "MMM d, h:mm a")}
              </span>
            </p>
                ...
                onClick={async () => {
                  try {
                    ...
                    toast({
                      title: "Post approved",
                      description:
                        "Your post has been approved and will be published soon.",
                    });
  ...
  const filteredPosts =
    socialPosts?.filter((post) => {
      ...
      if (activeTab === "responded")
        return status.toLowerCase() === "responded";
      return platform.toLowerCase() === activeTab.toLowerCase();
    }) || [];

  const pendingCount =
    socialPosts?.filter(
      (post) => getSocialData(post).status.toLowerCase() === "pending",
    ).length || 0;

  const respondedCount =
    socialPosts?.filter(
      (post) => getSocialData(post).status.toLowerCase() === "responded",
    ).length || 0;
      ...
      const checkForContent = async () => {
        ...
        if (attempts >= maxAttempts) {
          toast({
            ...
            description:
              "The post is still being generated. You can check back later in the social media section.",
            variant: "default",
          });
          setIsGenerating(false);
          return;
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "The post is being regenerated with your suggestions. This may take up to 30 seconds.",
      });
      ...
      const checkForContent = async () => {
        attempts++;
        console.log(
          `Checking for retry content, attempt ${attempts}/${maxAttempts}`,
        );
  ...
  const handleDialogClose = (forceClear = false) => {
    ...
    if (isGenerating && !forceClear) {
      toast({
        ...
        description:
          "Please wait for the generation to complete before closing.",
        variant: "destructive",
      });
      return;
    }
        ...
        >
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Use AI to create engaging social media content for your
                restaurant.
              </DialogDescription>
            </DialogHeader>
            ...
            {showSuggestionInput && !isGenerating ? (
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="suggestion">
                    How would you like to improve the post?
                  </Label>
                  <Textarea
                    id="suggestion"
              ...
              <div className="py-8 flex flex-col items-center justify-center">
                ...
                <p className="text-center text-xs text-neutral-500 mt-2">
                  The n8n workflow has been started. Your image and caption will
                  appear shortly.
                </p>
              </div>
            ) : !generatedContent ? (
              ...
              <>
                ...
                {!isGenerating ? (
                  ...
                  <div className="py-8 flex flex-col items-center justify-center">
                    ...
                    <p className="text-center text-xs text-neutral-500 mt-2">
                      The n8n workflow has been started. Your image and caption
                      will appear shortly.
                    </p>
                  </div>
                )}
                  ...
                  <div className="flex justify-between mt-4">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        console.log(
                          "Retry clicked - setting showSuggestionInput to true",
                        );
                      }}
                  ...
                  {platforms.map((platform) => (
                    <Badge
                      ...
                      className={cn(
                        ...
                        activeTab === platform.toLowerCase()
                          ? "border-[#2A4833]"
                          : "",
                      )}
          ...
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-neutral-500">
                No social media posts matching the selected filter.
              </p>
            </CardContent>
          </Card>
        ) : (
    ...
    reader.onloadend = async () => {
      try {
        ...
        toast({
          ...
          description:
            "Your image has been successfully uploaded to the gallery.",
        });
  ...
  const generateCaptionMutation = useMutation({
    mutationFn: async (id: number) => {
      ...
      const checkForCaption = async () => {
        ...
        const updatedPhoto = await fetch(`/api/gallery/${id}`).then((res) =>
          res.json(),
        );
        ...
        if (updatedPhoto && updatedPhoto.caption) {
          ...
          toast({
            ...
            description:
              "AI has successfully generated a caption for your image",
          });
          return;
        }
  ...
  const postPhotoMutation = useMutation({
    mutationFn: async (id: number) => {
      ...
      const checkForStatus = async () => {
        attempts++;
        console.log(
          `Checking for post status attempt ${attempts}/${maxAttempts}`,
        );
  ...
  const updateCaptionMutation = useMutation({
    mutationFn: async ({ id, caption }: { id: number; caption: string }) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error ||
              `Server returned ${response.status}: Failed to update caption`,
          );
        }
    ...
    onError: (error: any) => {
      ...
      toast({
        ...
        description:
          error.message || "There was a problem updating the caption",
        variant: "destructive",
      });
    },
  });
      ...
      fetch(`/api/gallery/${photoId}`, {
        ...
        .then(() => {
          ...
          setSelectedPhoto((prev) =>
            prev ? { ...prev, caption: caption } : null,
          );
        ...
        .catch((error) => {
          ...
          toast({
            title: "Failed to Update Caption",
            description:
              error.message || "An error occurred while updating the caption",
            variant: "destructive",
          });
        });
    } catch (err) {
