      ...
      <Tabs defaultValue="general" className="space-y-6">
        ...
        <TabsContent value="integrations">
          <div className="grid gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-2">
                  <Button 
                    ...
                    onClick={async () => {
                      try {
                        // Get the webhook URL from the input
                        const webhookUrlInput = document.getElementById('n8n-url') as HTMLInputElement;
                        const webhookUrl = webhookUrlInput?.value || "http://ec2-13-58-27-158.us-east-2.compute.amazonaws.com:5678/webhook/67eff4f0-a0e3-4881-b179-249a9394a340";
                        
                        // Create a simple test payload
                        const testPayload = {
                          test: true,
                          timestamp: new Date().toISOString(),
                          message: "Test connection from Restaurant AI Assistant"
                        };
                        
                        // Show toast indicating we're testing
                        toast({
                          title: "Testing connection",
                          description: "Sending test request to n8n webhook..."
                        });
                        
                        // Try the connection 
                        const response = await apiRequest("POST", "/api/proxy", testPayload);
                        const data = await response.json();
                        
                        // Show success message
                        toast({
                          title: "Connection successful!",
                          description: "Successfully connected to n8n webhook."
                        });
                        
                        console.log("n8n test connection response:", data);
                      } catch (error) {
                        // Show error message
                        toast({
            ...
            <Card>
              <CardHeader>
                <CardTitle>Webhook Test Tool</CardTitle>
                <CardDescription>Test connection to n8n webhook</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="grid gap-2">
                  ...
                  <div className="flex gap-2">
                    <Button 
                      onClick={async () => {
                        try {
                          const payload = document.getElementById('webhook-payload') as HTMLTextAreaElement;
                          
                          if (!payload || !payload.value) {
                            throw new Error('Please enter valid JSON payload');
                          }
                          
                          const jsonData = JSON.parse(payload.value);
                          
                          const startTime = new Date().getTime();
                          
                          // Update status
                          const statusEl = document.getElementById('webhook-test-status');
                          if (statusEl) statusEl.innerHTML = 'Sending request...';
                          
                          // Send to the primary proxy endpoint
                          const response = await apiRequest("POST", "/api/proxy", jsonData);
                          ...
                          if (statusEl) statusEl.innerHTML = `Success! Request completed in ${duration.toFixed(2)}s`;
                          
                          const responseEl = document.getElementById('webhook-test-response') as HTMLTextAreaElement;
                          if (responseEl) responseEl.value = JSON.stringify(responseData, null, 2);
                        } catch (error) {
                          const statusEl = document.getElementById('webhook-test-status');
                          if (statusEl) statusEl.innerHTML = `Error: ${error instanceof Error ? error.message : 'Unknown error'}`;
                        }
                      }}
                      variant="default"
                    >
                    ...
                    <Button 
                      onClick={async () => {
                        try {
                          ...
                          // Send to the alternative proxy endpoint
                          const response = await apiRequest("POST", "/api/proxy-alt", jsonData);
                          const responseData = await response.json();
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center">
                      <Label>Status:</Label>
                      <span id="webhook-test-status" className="ml-2 text-sm">Ready</span>
                    </div>
                    
                    <div className="grid gap-2">
                      <Label htmlFor="webhook-test-response">Response</Label>
                      <Textarea 
